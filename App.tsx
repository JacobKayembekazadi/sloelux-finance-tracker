...
const LogRow: React.FC<{ item: CombinedLogItem; index: number; onEdit: (item: CombinedLogItem) => void; onDelete: (item: CombinedLogItem) => void; }> = ({ item, index, onEdit, onDelete }) => {
    ...
    return (
        <tr className={index % 2 === 0 ? 'bg-gray-50' : 'bg-white'}>
            ...
            <td className="p-4 text-sm text-gray-600">{item.description} {isSale && (item as Transaction).quantity > 1 && `(x${(item as Transaction).quantity})`}</td>
...
const EditLogModal: React.FC<{ logItem: CombinedLogItem; onClose: () => void; onSave: (updatedItem: CombinedLogItem) => void; watchPrice: number; }> = ({ logItem, onClose, onSave, watchPrice }) => {
    ...
    const [quantity, setQuantity] = useState(logItem.type === 'sale' ? (logItem as Transaction).quantity.toString() : '1');
    const [error, setError] = useState('');
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (logItem.type === 'sale') {
            ...
            onSave({ ...logItem, date: newDate, description, quantity: numQuantity, amount: numQuantity * watchPrice } as Transaction);
        } else {
            const numAmount = parseFloat(amount);
            if (isNaN(numAmount) || numAmount <= 0) { setError('Please enter a valid positive amount.'); return; }
            if (!description.trim()) { setError('Description cannot be empty.'); return; }
            onSave({ ...logItem, date: newDate, description, amount: numAmount } as Expense);
        }
        onClose();
    };
...
const convertToAppFormat = (sheetsData: TransactionRow[]): { transactions: Transaction[], expenses: Expense[] } => {
...
// Main App Component with Google Sheets Integration
const AppWithGoogleSheets: React.FC<{
...
}> = ({ transactions: sheetsTransactions, addTransaction, updateTransaction, deleteTransaction, refreshTransactions, clearAllData }) => {
    ...
    const { transactions, expenses } = useMemo(() => 
        convertToAppFormat(sheetsTransactions), [sheetsTransactions]
    );
    ...
    const handleAddSale = async (sale: SaleData) => {
    ...
    const handleUpdateLog = async (updatedItem: CombinedLogItem) => {
        const sheetsFormat: TransactionRow = {
            id: updatedItem.id,
            date: updatedItem.date,
            amount: updatedItem.amount,
            type: updatedItem.type,
            quantity: updatedItem.type === 'sale' ? (updatedItem as Transaction).quantity : undefined,
            description: updatedItem.description
        };
        
        await updateTransaction(sheetsFormat);
        setEditingLog(null);
    };
    
    const handleDeleteLog = async (logItem: CombinedLogItem) => {
        if (window.confirm(`Are you sure you want to delete this ${logItem.type}? This action cannot be undone.`)) {
            await deleteTransaction(logItem.id);
        }
    };
    ...
    const chartData = useMemo(() => {
        ...
        chronologicalLog.forEach((item) => {
            if (item.type === 'sale') {
                ...
                cumulativeProfit += (item.amount - ((item as Transaction).quantity * settings.watchCost));
            } else {
                    ...
                    <header className="mb-8 flex justify-between items-center">
                        ...
                        <div className="flex space-x-2">
                            <button
                                ...
                                className="bg-white text-gray-700 hover:bg-gray-200 px-4 py-2 rounded-lg flex items-center transition-colors"
                            >
...
export default function App() {
    return (
        <GoogleSheetsManager>
            {(googleSheetsProps) => <AppWithGoogleSheets {...googleSheetsProps} />}
        </GoogleSheetsManager>
    );
}
