...
export interface UseGoogleSheetsOAuth2Result {
...
export const useGoogleSheetsOAuth2 = (): UseGoogleSheetsOAuth2Result => {
  const [transactions, setTransactions] = useState<TransactionRow[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [sheetsService, setSheetsService] = useState<GoogleSheetsService | null>(null);
  const [oauthService, setOauthService] = useState<GoogleOAuth2Service | null>(null);
  ...
  useEffect(() => {
    try {
      validateConfig();
      const oauth = new GoogleOAuth2Service(GOOGLE_OAUTH2_CONFIG);
  ...
  // Initialize OAuth2 client when service is ready
  useEffect(() => {
    if (oauthService) {
      oauthService.initialize().catch((err: Error) => {
  ...
  const signIn = useCallback(async () => {
    ...
    try {
      ...
      const sheetsConfig = oauthService.getGoogleSheetsConfig();
